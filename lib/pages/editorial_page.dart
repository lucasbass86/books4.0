...
class EditorialPage extends StatefulWidget {
  static const String routeName = 'EditorialPage';
...
class _EditorialPageState extends State<EditorialPage> {
  ...
  Widget build(BuildContext context) {
    ...
    editorial = ModalRoute.of(context)!.settings.arguments as Editorial;
    List<EditorialBook> editorialBooks =
        servicio.editorialesBooks.where((e) => e.idEditorial == editorial.id).toList();
    final libroMap = {
      for (var l in servicio.libros) l.codigo.toString(): l,
    };
    libros = editorialBooks.map((c) => libroMap[c.idLibro]).whereType<Libro>().toList();
    if (!_isSearching) {
      switch (typeFilter) {
                ...
                Row(
                  ...
                  children: [
                    CircleAvatar(
                        ...
                        child: Text(editorial.descripcion.substring(0, 1))),
                    Expanded(
                      child: Text(
